...
// wartość na 100 - tak żaby parametry z valuesetów się dobrze wyświetlały
var EXCESS_ROWS = 100;
...
var ngMoveSelectionHandler = function($scope, elm, evt, grid) {
    ...
    if (grid.config.customKeys) {
        if (grid.config.customKeys.keys.indexOf(charCode) >= 0) {
                grid.config.customKeys.listener(evt);
                evt.stopPropagation();
                return;
        }
    }
...
angular.module('ngGrid.services').factory('$domUtilityService',['$utilityService', '$window', function($utils, $window) {
    ...
    domUtilityService.UpdateGridLayout = function($scope, grid) {
        ...
        if (grid.config.aggregated) {
            var iiiii = 12;
//            grid.elementDims.rootMaxW = grid.elementDims.rootMaxW - 25;
        }
...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    ...
    self.configureColumnWidths = function() {
        ...
        if (asterisksArray.length > 0) {
            ...
            var outerWidth = self.rootDim.outerWidth;
            if (self.config.aggregated) {
                var groupingSub = 38;
                remainingWidth = remainingWidth - groupingSub;
                outerWidth = outerWidth - groupingSub;
            }
            ...
            angular.forEach(asterisksArray, function(colDef, i) {
                ...
                if(isLast && totalWidth < outerWidth){
                    var gridWidthDifference = outerWidth - totalWidth;
                    if(self.maxCanvasHt > $scope.viewportDimHeight()){
